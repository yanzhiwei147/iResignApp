...
module.exports = class SignApp {
  ...
  prepare() {
    ...
    let keychain = this.config["keychain"];
    ...
    let ipaFile = ipa
    if (!isDirectory(ipaFile)) { 
      ipaFile = path.join(currentWorkingDirectory, ipaFile);
    }

    if (output.length == 0) {
      output = path.join(path.basename(ipaFile, path.extname(ipaFile)));
      output = output + "-resigned" + path.extname(ipaFile);
    }
    if (!isDirectory(output)) {
      output = path.join(currentWorkingDirectory, output);
    }

    if (!isDirectory(mobileprovision)) {
      mobileprovision = path.join(currentWorkingDirectory, mobileprovision);
    }
    // console.log("keychain:" + keychain + "\n" + "ipa:" + ipaFile + "\n" + "identity:" + identity + "\n" +"output:" + output + "\n" + "mobileprovision:" + mobileprovision + "\n" + "bundleid:" + bundleId + "\n");

    const signFile = path.join(__dirname, 'sign.sh');
    return {
      keychain: keychain,
      ipa: ipa,
      identity: identity,
  ...
  executeResign(configs, cb) {
    ...
    const args = [signFile, ipaFile, identity, mobileprovision, output];
